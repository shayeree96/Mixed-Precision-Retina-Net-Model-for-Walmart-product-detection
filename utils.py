...
def vis_detection(im, boxes, scores, vis_thresh=0.5):
    plt.imshow(im)
    for box, score in zip(boxes, scores):
        if score > vis_thresh:
            rect = plt.Rectangle((box[0], box[1]),
                                 box[2] - box[0] + 1,
                                 box[3] - box[1] + 1, fill=False,
                                 edgecolor=(0, 1, 0), linewidth=2)
            plt.gca().add_patch(rect)
            plt.gca().text(box[0], box[1], '{:.3f}'.format(score),
                           fontsize=12, color='white')
    plt.show()


def img_resize(img, scale, max_size=None):
    w, h = img.size
    short_edge = min(w, h)
    long_edge = max(w, h)
    factor = float(scale) / float(short_edge)
    if scale == 0:
        factor = 1.
    if max_size is not None:
        if factor * long_edge > max_size:
            factor = float(max_size) / float(long_edge)
    w_new = int(round(w * factor))
    h_new = int(round(h * factor))
    return img.resize((w_new, h_new), resample=Image.BILINEAR), factor
